...
class A4NN:
    ...
    def __init__(self):
        ...
        self.X = 2*np.loadtxt(path+"/train_dataset.csv",delimiter=",")/45 - 1
    ...
    def predict(self,X1):
        ...
        l0[0] = 2*np.asanyarray(X1, dtype=np.float32)/45 - 1
        l1 = sigmoid(np.dot(l0, self.syn0))
        l2 = sigmoid(np.dot(l1, self.syn1))
        l3 = sigmoid(np.dot(l2, self.syn2))
...
def test():
    net = A4NN()
    net.train()
    test_dataset = [30,30,30,30,30,30,30,30,30,30,30,30,30,14,14,14,14,14,14,14,14,14,14,14,14,11,11,11,11,11,11,11,11,11,11,11,10,10,10,10,10,12,14,10]
    result = net.predict(test_dataset)
    print("test-set validation.  Expected: ~0 - Result:" + repr(result))
    test_dataset = [33,33,33,33,33,33,33,33,40,40,40,40,41,40,40,40,41,40,40,40,41,40,40,40,41,40,40,40,41,40,40,40,41,40,40,40,41,40,40,40,41,40,40,40]
    result = net.predict(test_dataset)
